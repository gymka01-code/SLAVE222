...
class EscapeGame:
    def __init__(self):
        self.round_id = 0; self.status = "waiting"; self.mult = 1.00; self.start_time = 0.0; self.crash_point = 1.00; self.history = []; self.bets: Dict[int, Dict[str, Any]] = {}; self.connections: List[WebSocket] = []
...
INSERT INTO jobs (title, min_yield, max_yield, drop_chance, emoji) VALUES ('Подметать полы', 0.15, 0.20, 70, '🧹'), ('Раздавать листовки', 0.175, 0.225, 70, '📄'), ('Майнить крипту', 0.225, 0.275, 25, '⛏'), ('Петь на улице', 0.20, 0.25, 25, '🎤'), ('Тапать хомяка', 0.30, 0.35, 5, '🐹'), ('Просить милостыню', 0.275, 0.325, 5, '🙏');
...
async def rate_limit(key: str, limit: int, window: int):
    try:
        ...
        if (await pipe.execute())[0] > limit: raise HTTPException(429, "Слишком много запросов.")
...
async def _full_profile(uid: int) -> dict:
    ...
    return {
        ...
        "owner": owner, "custom_name": u['custom_name'], "slaves": slaves, "slaves_count": len(slaves), "max_slaves": max_slaves, 
...
async def collect_income(owner_id: int) -> float:
    ...
    clan_level = await db.fetchval("SELECT level FROM syndicates WHERE id=$1", owner_info['syndicate_id']) or 1 if owner_info['syndicate_id'] else 1
    total = 0.0
    for s in slaves:
...
async def lifespan(app: FastAPI):
            ...
            try:
                ...
                if last_tax_cron != date_str and msk_time.hour >= 0:
                    ...
                    for tu in tax_users:
                        ...
                        if float(tu['balance']) >= tax: await db.execute("UPDATE users SET balance = balance - $1::numeric, last_tax_date = $2 WHERE id = $3", tax, date_str, tu['id'])
                        else:
    ...
    yield
    await db.close(); await rdb.aclose()
...
async def heal_slave(req: HealReq, user: dict = Depends(get_current_user)):
    ...
    await db.execute("UPDATE users SET balance=balance-500, is_injured_until=NULL WHERE id=$1", req.slave_id)
...
async def resolve_robbery(req: RobberyReq, user: dict = Depends(get_current_user)):
    ...
    if req.status == "caught":
        ...
        stolen = float(target['balance']) * (min(req.amount, 5.0) / 100.0)
        if stolen > 0:
...
async def get_clan_chat_history(user: dict = Depends(get_current_user)):
    ...
    rows = await db.fetch("SELECT sm.id, sm.user_id, u.first_name, u.photo_url, u.syndicate_role, sm.text, sm.created_at FROM syndicate_messages sm JOIN users u ON u.id = sm.user_id WHERE sm.syndicate_id=$1 ORDER BY sm.created_at DESC LIMIT 50", user['syndicate_id'])
...
async def send_clan_message(req: SyndicateChatSendReq, user: dict = Depends(get_current_user)):
    ...
    await clan_ws.send_to_user(user['syndicate_id'], {"type": "new_message", "message": {"id": msg_id, "user_id": user['id'], "first_name": user['first_name'], "photo_url": user['photo_url'], "syndicate_role": user['syndicate_role'], "text": text, "created_at": datetime.utcnow().isoformat() + "Z"}})
    return {"ok": True}
...
async def get_syndicates(user: dict = Depends(get_current_user)):
    rows = await db.fetch("SELECT s.*, u.username as owner_username, u.first_name as owner_name, (SELECT COUNT(*) FROM users WHERE syndicate_id=s.id) as members_count FROM syndicates s LEFT JOIN users u ON u.id = s.owner_id ORDER BY s.treasury DESC LIMIT 50")
    return [dict(r) for r in rows]
